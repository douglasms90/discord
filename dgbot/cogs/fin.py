...
class fin(commands.Cog):
    ...
    async def assets(self, ctx):
        if ctx.author.id in [269592803602989058]:  # D
            ...
            async def sync(interaction: discord.Interaction):
                ...
                allassets = self.readIn("SELECT * FROM assets order by id asc")
                for i in allassets:
                    if i[1] == 'rf':
                        pass
                    elif i[1] == 'dollar':
                        ...
                        self.crudIn(f"UPDATE assets SET pr = {pr} WHERE id = {i[0]}")
                        if i[1] == 'fundos-imobiliarios':
                            ...
                            try:
                                ...
                                vp = 0
                            self.crudIn(f"UPDATE assets SET dv = {dv}, vp = {vp} WHERE id = {i[0]}")
                        if i[1] == 'acoes':
                            dv = float((st.find_all('span', class_='sub-value')[3].text)[3:].replace(',', '.'))
                            ...
                            self.crudIn(f"UPDATE assets SET dv = {dv}, pl = {pl}, vp = {vp} WHERE id = {i[0]}")
                await interaction.followup.send("Sync ok", ephemeral=True)
            
            async def rf(interaction: discord.Interaction):
                dump = ""
                tta = ttc = tc = ta = 0
                active = self.readIn("SELECT * FROM assets order by id asc")
                for i in active:
            ...
            async def acoes(interaction: discord.Interaction):
                ...
                for i in active:
                    if i[1] == "acoes":
                        dump += f"{i[0]}{i[2].upper():>7}{'%.2f' %(((i[3]-i[4])/i[4])*100):>7}{'%.2f' %(i[3]):>6}{'%.2f' %(i[4]):>6}{'%.0f' %(i[5]):>4}{'%.2f' %((i[6]/i[3])*100):>6}{'%.2f' %((i[6]/i[4])*100):>6}{i[7]:>6}{i[8]:>5}{'%.2f' %(i[3]*i[5]):>9}{'%.2f' %(i[4]*i[5]):>9}{'%.2f' %((i[3]*i[5])-(i[4]*i[5])):>8}\n"
                        tc += i[3]*i[5]
                        ta += i[4]*i[5]
                        dv += i[6]
                        dp += (i[6]/i[3])*100
                        yp += (i[6]/i[4])*100
                        ct += 1
                dump += f"id{'nm':>6}{'vl%':>8}{'pr':>5}{'pm':>6}{'qt':>6}{'%.2f' %(dp/ct):>6}{'%.2f' %(yp/ct):>6}{'pl':>5}{'vp':>5}{'%.2f' %(tc):>10}{'%.2f' %(ta):>9}{'%.2f' %(tc-ta):>8}\n"
                await interaction.response.send_message(f"```{dump}```", ephemeral=True)
            ...
            async def ttl(interaction: discord.Interaction):
                ...
                dump += f"tl {'%.2f' %((fdp+adp)/(fct+act))}{'%.2f' %((fyp+ayp)/(fct+act)):>7}{'%.2f' %(ttc):>20}{'%.2f' %(tta):>10}{'%.2f' %(ttc-tta):>9}\n"
                await interaction.response.send_message(f"```{dump}```", ephemeral=True)
    ...
    @commands.command(name="assetsreplace")
    async def replace(self, ctx, *args):
        if ctx.author.id in [269592803602989058]:  # D
            ...
            before = self.readIn(f"SELECT * FROM assets WHERE id = {args[0]}")

            async def y(interaction: discord.Interaction):
                self.crudIn(f"UPDATE assets SET {args[1]} = {args[2].replace(',','.')} WHERE id = {args[0]}")
                after = self.readIn(f"SELECT * FROM assets WHERE id = {args[0]}")
    ...
    @commands.command(name="assetsdelete")
    async def delete(self, ctx, *args):
        if ctx.author.id in [269592803602989058]: # D
            ...
            async def y(interaction: discord.Interaction):
                self.crudIn(f"DELETE FROM assets WHERE id = {args[0]}")
